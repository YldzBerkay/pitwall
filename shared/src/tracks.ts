...
export interface Track {
  ...
  pitLossSec: number;
  /**
   * Pit yolundan geçmenin normal tura göre maliyeti, saniye. Parc fermé'yi
   * ihlal eden araç grid yerini almaz; saha geçtikten sonra pit çıkışından
   * katılır ve bu kadar ek boşlukla yarışa başlar (bkz. spec §7.1).
   *
   * Her zaman `pitLossSec`'ten küçüktür: pit durağı bu transitin üstüne bir de
   * lastik değişimini ekler. Pistten piste değişir, çünkü asıl belirleyen
   * pit yolunun tura oranıdır: dar, düşük hız limitli sokak pit yolları kısa
   * turun beşte birini yer (Principality, Marina Lights), uzun turlu hız
   * pistlerinde aynı transit turun yalnızca sekizde biridir (Ardennes).
   * Tek bir sabit sayı olsaydı pist seçiminin cezaya etkisi kalmazdı.
   */
  pitLaneSec: number;
...
const t = (
  ...
  sprint: boolean,
  region: TrackRegion,
): Track => ({ key, gp, country, circuit, demand, label, rainChance, attrition, overtaking, laps, baseLapSec, pitLossSec, pitLaneSec, layout, raceControl, sprint, region });

export const calendar: Track[] = [
  t('dunes', 'Gulf GP', 'BHR', 'Al Rimal Desert Circuit', { motor: 0.4, aero: 0.25, grip: 0.35 }, 'Karışık · fren', 0.02, 0.35, 0.75, 57, 91, 21, 16.5,
  ...
  t('corniche', 'Red Sea GP', 'SAU', 'Corniche Lights Street Circuit', { motor: 0.4, aero: 0.4, grip: 0.2 }, 'Sokak pisti · hız', 0.03, 0.6, 0.55, 50, 89, 20, 17.5,
  ...
  t('harbourpark', 'Southern Cross GP', 'AUS', 'Harbour Park Circuit', { motor: 0.3, aero: 0.35, grip: 0.35 }, 'Park pisti · karışık', 0.25, 0.45, 0.5, 58, 80, 19, 14,
  ...
  t('kashima', 'Rising Sun GP', 'JPN', 'Kashima Figure-Eight', { motor: 0.25, aero: 0.45, grip: 0.3 }, 'Aero · akıcı virajlar', 0.35, 0.3, 0.4, 53, 92, 22, 14.5,
  ...
  t('pearlriver', 'Pearl River GP', 'CHN', 'Pearl River International', { motor: 0.4, aero: 0.3, grip: 0.3 }, 'Motor gücü · uzun düzlük', 0.2, 0.3, 0.7, 56, 94, 22, 15,
  ...
  t('bayfront', 'Bayfront GP', 'USA', 'Bayfront Stadium Circuit', { motor: 0.35, aero: 0.3, grip: 0.35 }, 'Sokak pisti · karışık', 0.3, 0.5, 0.6, 57, 88, 21, 18,
  ...
  t('valledoro', 'Riviera GP', 'ITA', "Valle d'Oro Autodromo", { motor: 0.3, aero: 0.4, grip: 0.3 }, 'Klasik pist · aero', 0.25, 0.4, 0.3, 63, 78, 23, 12.5,
  ...
  t('principality', 'Principality GP', 'MCO', 'Rocher Harbour Streets', { motor: 0.1, aero: 0.35, grip: 0.55 }, 'Sokak pisti · yol tutuş', 0.2, 0.7, 0.05, 78, 72, 18, 15.5,
  ...
  t('montjuic', 'Iberian GP', 'ESP', 'Sierra Blanca Circuit', { motor: 0.3, aero: 0.45, grip: 0.25 }, 'Aero · test pisti', 0.1, 0.25, 0.5, 66, 79, 21, 13,
  ...
  t('islandloop', 'Maple GP', 'CAN', 'Île du Nord Loop', { motor: 0.45, aero: 0.2, grip: 0.35 }, 'Motor gücü · duvarlar yakın', 0.3, 0.55, 0.65, 70, 74, 19, 13.5,
  ...
  t('alpenring', 'Alpine GP', 'AUT', 'Alpenring', { motor: 0.45, aero: 0.3, grip: 0.25 }, 'Motor gücü · kısa tur', 0.3, 0.3, 0.7, 71, 66, 20, 10.5,
  ...
  t('greystone', 'Albion GP', 'GBR', 'Greystone Aerodrome', { motor: 0.3, aero: 0.5, grip: 0.2 }, 'Aero · hızlı virajlar', 0.4, 0.3, 0.55, 52, 87, 22, 13,
  ...
  t('danubering', 'Danube GP', 'HUN', 'Danube Ring', { motor: 0.2, aero: 0.35, grip: 0.45 }, 'Yol tutuş · dar pist', 0.25, 0.35, 0.2, 70, 76, 20, 13.5,
  ...
  t('ardennes', 'Ardennes GP', 'BEL', 'Ardennes Forest Circuit', { motor: 0.4, aero: 0.4, grip: 0.2 }, 'Motor gücü · aero', 0.5, 0.45, 0.7, 44, 106, 24, 14,
  ...
  t('duinen', 'Lowlands GP', 'NLD', 'Duinen Beach Circuit', { motor: 0.2, aero: 0.4, grip: 0.4 }, 'Yol tutuş · eğimli virajlar', 0.35, 0.4, 0.25, 72, 71, 19, 13,
  ...
  t('velocita', 'Lombardy GP', 'ITA', 'Autodromo Velocità', { motor: 0.55, aero: 0.25, grip: 0.2 }, 'Motor gücü · hız tapınağı', 0.2, 0.4, 0.8, 53, 81, 23, 14,
  ...
  t('caspian', 'Caspian GP', 'AZE', 'Caspian Old Town Circuit', { motor: 0.5, aero: 0.2, grip: 0.3 }, 'Sokak pisti · motor gücü', 0.1, 0.65, 0.6, 51, 100, 21, 18.5,
  ...
  t('marinalights', 'Straits GP', 'SGP', 'Marina Lights Night Circuit', { motor: 0.15, aero: 0.35, grip: 0.5 }, 'Sokak pisti · yol tutuş', 0.4, 0.75, 0.15, 62, 96, 22, 19.5,
  ...
  t('prairie', 'Lone Star GP', 'USA', 'Prairie Hill Circuit', { motor: 0.3, aero: 0.4, grip: 0.3 }, 'Karışık · akıcı', 0.2, 0.3, 0.6, 56, 95, 22, 15,
  ...
  t('altiplano', 'Altiplano GP', 'MEX', 'Altiplano Stadium Circuit', { motor: 0.45, aero: 0.2, grip: 0.35 }, 'Motor gücü · yüksek irtifa', 0.15, 0.4, 0.65, 71, 78, 20, 13.5,
  ...
  t('serrahill', 'Serra GP', 'BRA', 'Serra Hill Circuit', { motor: 0.35, aero: 0.3, grip: 0.35 }, 'Karışık · yağmur riski', 0.55, 0.45, 0.7, 71, 72, 20, 12,
  ...
  t('neonstrip', 'Neon GP', 'USA', 'Neon Boulevard Circuit', { motor: 0.55, aero: 0.2, grip: 0.25 }, 'Sokak pisti · motor gücü', 0.05, 0.5, 0.7, 50, 98, 21, 18.5,
  ...
  t('lagoon', 'Lagoon GP', 'ARE', 'Lagoon Marina Circuit', { motor: 0.35, aero: 0.35, grip: 0.3 }, 'Karışık · sezon finali', 0.02, 0.3, 0.45, 58, 86, 21, 15,
