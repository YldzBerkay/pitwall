...
- 🔶 **§3.4'ün %85/%75 hedefi tutmuyor — karar spec sahibinde.** "En güçlü n. araç" = güç sırasındaki n. TAKIM (bir koltuk = bir takım = iki araç). 11 koltuklu ızgarada dürüst tavan `(11 − n) / (11 − 1)` = **%80 / %70**; ölçülen **~%76 / %66** ve bu havuzun sunabildiğinin tamamı. Sebep seçim algoritması değil, ekosistem: lobiyi dolduran 10 katılımcının ilk n−1'i, n. takım HENÜZ BOŞKEN gelmek zorunda, yoksa lobi hiç büyümez. Hedefe ulaşmanın tek yolu boş koltuğu dolu göstermek olurdu, §3.4 bunu açıkça yasaklıyor. Seçenekler: hedefleri tavanın altına çekmek (~%80/%70) ya da koltuğu takım yerine tek araç yapmak (22 koltukta tavan %90/%86). `npm run sim:matchmaking` bu farkı raporluyor ve başarısız sayıyor.
